...
class AdminGoodsController extends BaseProjectAdminController {
	...
	async getAdminGoodsList() {
		...
		for (let k = 0; k < list.length; k++) {
			...
			list[k].GOODS_RENT_TIME = timeUtil.timestamp2Time(list[k].GOODS_RENT_TIME, 'Y-M-D h:m:s');
	...
	async insertGoods() {
		...
		// 数据校验 
		let rules = {
			title: 'must|string|min:2|max:100|name=标题',
	...
	async goodsOrderDataGet() {
		...
		if (input.isDel === 1)
			await service.deleteGoodsOrderDataExcel(); //先删除

		return await service.getGoodsOrderDataURL();
	}

	/** 导出数据 */
	async goodsOrderDataExport() {
		await this.isAdmin();
		...
		let service = new AdminGoodsService();
		return await service.exportGoodsOrderDataExcel(input);
	}

	/** 删除导出的借用数据文件 */
	async goodsOrderDataDel() {
		await this.isAdmin();
		...
		let service = new AdminGoodsService();
		return await service.deleteGoodsOrderDataExcel();
	}
